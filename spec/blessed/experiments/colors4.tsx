import { installExitKeys, Box, debug, React, BoxOptions, createScreen, box } from '../../../src';
...
function test3() {
  ...
  for (let y = 0; y < M; y += 1) {
    ...
    const options = { top: calcY(y), left: 0,  content:  `${paletteName}`,  width: dx, height: dy, style: { bg: 'white', bold: true, underline: true, fg: 'black' }, hoverText: `Palette: \n${paletteName}, ${Object.keys(Object.values(palettes)[y]).length} colors` }
  ...
  for (let y = 0; y < M; y += 1) {
    for (let x = 1; x < N; x += 1) {
      ...
      const options : BoxOptions= { top: calcY(y), 
        // border: 'line', 
        focusable: true, 
        mouse: true, clickable: true, keyable: true, keys: true,
input: true,
    tags: true,
    vi: true,

         focused: true, draggable: true,
        left: calcX(x)-1,    content: colorName,  width: dx, height: dy, 
        style: { bg: color, fg: invertColor(color), bold: true,  hover: {bg: color, fg:invertColor(color)}, focus: {bg: color, fg:invertColor(color)},
