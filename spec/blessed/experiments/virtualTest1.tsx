import { Br, Component, createScreen, Div, installExitKeys, React, Screen, ListTable as BlessedListTable } from '../../../src'
...
class ListTable extends Component<ListTableProps> {
  ...
  render() {
    ...
    const tds = getJSXChildrenProps(this)!.find(e=>e.tagName==='TBody')!.children.filter(c=>(c as any).tagName==='Tr').map(c=>(c as any).children.map((c:any)=>c.children.join('')))
    ...
    return (
      <Div>
        ...
        <listtable ref={this.listTableRef}width="100%" height="80%" border="line" data={[ths, ['asd', 'sdf', 'dfg']].concat(tds)}>
