...
It's a demonstration of Google-palette (https://github.com/google/palette.js) color palettes in the terminal. <Br/><Br/>
  * Use [UP] and [DOWN] keys to scroll the content. <Br/>
  * Also you can use the mouse wheel to scroll. <Br/>
  * At any time you can close the demo with keys [Q], or [Control-c] or [ESC]. <Br/><Br/>
  * Close this modal with keys [Q], or [Control-c] or [ESC]. <Br/><Br/><Br/>
  Enjoy.<Br/><Br/>
...
Project Home: (https://github.com/cancerberoSgx/accursed/) <Br/>
  ...
  Object.keys(palettes).forEach(title => {
    ...
    Object.keys(palettes[title]).forEach(paletteName => {
      ...
      x += dx * 2
      palettes[title][paletteName].map(color => '#' + color).forEach(color => {
