...
import { installExitKeys, Box, debug } from '../../../src';
...
if(priority==='b'){
  ...
  for (r = 0; r < 255 - step + 1; r += step) {
    for (g = 0; g < 255 - step + 1; g += step) {
      blessed.box({parent: screen, left: roundX(g),  top: roundY(r), width: roundX(step), height: roundY(step), style: {bg:buildColor(r, g, 'r') }
      })
