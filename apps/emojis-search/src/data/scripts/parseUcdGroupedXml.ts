...
import { notFalsy, uniqueNotFalsy } from '../../util'
...
export function getCharsDefinitions(chars: CharWithGroup[], blocks: Partial<Block_>[], groups: Partial<Group_>[]) {
  const charsWithBlockAndGroupIndex = chars
    .map(c => {
...
export function toTs(
  chars: CharWithGroup[],
  blocks: Partial<Block_>[],
  groups: Partial<Group_>[],
  filteredBlocks: string[] = blocks.map(b => b.name).filter(notFalsy)
) {
...
export enum Unicode12 {
  ${filteredChars
    ...
    .map(c => `${printUnicodeCharacter(c.cp!, true)}`)
    .join(', \n')}
}
${filteredChars
  .map(c => c.blockName)
  .filter(uniqueNotFalsy)
  .map(
    b => `
...
function getCharsWithBlockAndGroupIndexDocumentTest(
  chars: CharWithGroup[],
  blocks: Partial<Block_>[],
  groups: Partial<Group_>[]
) {
...
export function getCharsWithBlockAndGroupIndexDocument(
  chars: CharWithGroup[],
  blocks: Partial<Block_>[],
  groups: Partial<Group_>[]
) {
...
function printTypeSamples(obj: Element) {
  const groups = obj
    .elements!.find(e => e.name === 'ucd')!
    .elements!.find(e => e.name === 'repertoire')!
    .elements!.filter(e => e.name === 'group')
  const blocks = obj
    .elements!.find(e => e.name === 'ucd')!
    .elements!.find(e => e.name === 'blocks')!
    .elements!.filter(e => e.name === 'block')
  // function findChar
  ...
  console.log(
    'char',
    JSON.stringify({
      ...((findWithMostAttributes(groups.map(g => g.elements!.filter(e => e.name === 'char')!).flat()) || {})
        .attributes || {})
    })
  )
}
