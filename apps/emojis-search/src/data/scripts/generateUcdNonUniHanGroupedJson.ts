...
import { Block, CharWithGroup, getCharsDefinitions, Group } from './parseUcdGroupedXml'
...
function generateUcdNonUniHanGroupedJson() {
  ...
  const blocks = obj
    ...
    .map(b => b.attributes as Block)
  const chars = groupElements
    .map(g =>
      (g.elements || [])
        .filter(e => e.name === 'char')
        .map(char => ({ ...char.attributes, group: g.attributes as Group } as CharWithGroup))
    )
    .flat()
