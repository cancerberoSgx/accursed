import { ArtificialEvent, Box, Br, Component, Div, Element, isElement, List as ListElement, React, input, showInModal, Strong } from 'accursed'
import { inputOptions } from './elementOptions'
import { getCategoryEmojis, List } from './list'
import { emojiDescriptions, EmojiDefinition } from './data/emojis';
  ...
  render() {
    return (
      <Div width="100%" height="100%">
        To search, focus the textbox below and press [enter] to start writing. [enter] for search. [esc] to restore focus navigation :
        <Br />
        <textbox {...inputOptions()} width="90%" padding={1} label="Search Query" height={5} input={true} {          ...{value: 'ball'}        }
        on={['submit', (value: string)=>{
          if(this.blessedElement && this.blessedElement.screen){
            const v = value.toLowerCase().trim()
            const emojis = Object.keys(emojiDescriptions).filter(k=>{
              const e = (emojiDescriptions as any)[k] as EmojiDefinition
              return JSON.stringify([e.name, e.short_name, e.text, e.texts&&e.texts.join(' '), e.short_names && e.short_names.join(' '),e.au].filter(T=>T).join(' ')).includes(v) // TODO: do this better
            })
    ...
    setTimeout(() => {
      container.append(React.render(<Div>
